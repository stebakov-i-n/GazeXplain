...
class CrossAttentionPredictor(nn.Module):
    def __init__(self, hidden_dim=2048, d_model=512, nhead=8, dropout=0.4, activation='relu'):
        ...
        self.norm_vlm = nn.LayerNorm(hidden_dim)
        ...
        self.querypos_embed1 = nn.Embedding(1200, d_model)
        self.querypos_embed2 = nn.Embedding(768, d_model)
    ...
    def forward(self, tgt, memory, tgt_mask: Optional[Tensor] = None,
        ...
        memory = self.proj(self.norm_vlm(memory)).permute(1, 0, 2)

        q = k = v = self.with_pos_embed(memory, self.querypos_embed1.weight.unsqueeze(1))
        memory2 = self.self_attn(q, k, value=v, attn_mask=tgt_mask,
                              key_padding_mask=tgt_key_padding_mask)[0]
        memory = memory + self.dropout(memory2)
        memory = self.norm1(memory)
        att = self.multihead_attn(query=patchpos_embed(tgt) if patchpos_embed else tgt, 
                                  key=self.with_pos_embed(memory, self.querypos_embed1.weight.unsqueeze(1)),
                                  value=memory, attn_mask=memory_mask,
                                  key_padding_mask=memory_key_padding_mask)[0]
        tgt = tgt + self.dropout2(att)
        tgt = self.norm2(tgt)
...
class Transformer(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.args.vlm_based:
            self.cross_att = CrossAttentionPredictor(self.args.img_hidden_dim, self.args.hidden_dim, self.args.nhead, self.args.dropout, self.args.activation)
        ...
        encoder_layer = TransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                encoder_dropout, activation, normalize_before, self.args)
        ...
        self.encoder = TransformerEncoderWrapper(encoder, input_proj, img_transform, text_transform, encoder_dropout, self.args)
    ...
    def forward(self, src: Tensor, tgt: Tensor, task: Tensor, src_vlm=None, src_mask: Optional[Tensor] = None,
...
class TransformerEncoderWrapper(nn.Module):
    ...
    def __init__(self, encoder, input_proj, img_transform, text_transform, dropout, args):
        super().__init__()
        self.args = args
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward_post(self, tgt, memory,
        ...
        tgt2 = self.multihead_attn(query=self.with_pos_embed(tgt, querypos_embed),
                                   key=patchpos_embed(memory) if patchpos_embed is not None else memory,
                                   value=memory, attn_mask=memory_mask,
                                   key_padding_mask=memory_key_padding_mask)[0]
