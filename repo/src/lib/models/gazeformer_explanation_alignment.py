...
class gazeformer(nn.Module):
    def __init__(self, transformer, args):
        ...
        # self.patchpos_embed = None if self.args.vlm_based else PositionEmbeddingSine2d(spatial_dim, hidden_dim=self.hidden_dim, normalize=True)

        # classify fixation, or PAD tokens
        self.token_predictor = nn.Linear(self.hidden_dim, 1)
        self.action_predictor = nn.Linear(self.hidden_dim, 768)
    ...
    def training_process(self, batch):
        ...
        if self.args.vlm_based:
            src_vlm = batch["image_feature_vlm"]
        else:
            src_vlm = None

        if self.args.vlm_based:
            task = torch.zeros(src.shape[0], 768).to(src.device)
        else:
        ...
        memory, memory_task, outs = self.transformer(src=src, tgt=tgt_input, src_vlm=src_vlm, tgt_mask=None, tgt_key_padding_mask=None,
        ...
        # if self.args.vlm_based:
        #     action_map = self.action_predictor(outs).permute(1, 0, 2)
        # else:
        action_map = self.attention_map_predictor(outs, memory if self.args.vlm_based else memory_task, tgt_mask=None, memory_mask=None,
        ...
        if self.args.explanation:
            ...
            with torch.no_grad():
                dec_outputs = self.blip_model(**dec_input)
            language_similarity = torch.zeros_like(resnet_visual_similarity)
            multimodal_similarity = torch.zeros_like(resnet_visual_similarity)
