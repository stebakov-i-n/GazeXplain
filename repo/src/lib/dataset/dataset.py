...
class UnifiedScanpath(Dataset):
    ...
    def __init__(self, split, opt, datasets=None):
        ...
        if self.opt.search_dataset == "GazeSearch":
            for i in range(len(cocosearch18_TP_fixations)):
                cocosearch18_TP_fixations[i]["T"] = [j * 1000 for j in cocosearch18_TP_fixations[i]["T"]]

        cocosearch18_TP_fixation_desc_dir = os.path.join(self.opt.dataset_dir, "COCO", "TP", "processed", "Qwen3_30B_PROMPT_L.json")
        with open(cocosearch18_TP_fixation_desc_dir, "r") as f:
        ...
        for explanation in (cocotp_explanations if self.opt.explanation else cocosearch18_TP_fixations):
            if not self.opt.explanation:
                explanation = {'name': explanation['name'],
                            'subject': explanation['subject'],
                            'task': explanation['task'],
                            'explanation': ['Desc.' for i in range(len(explanation["X"]))]}
    ...
    def __getitem__(self, idx):
        ...
        if self.opt.vlm_based:
            image_ftrs_vlm = torch.load(img_path.replace(".pth", "_vlm.pth")).to(image_ftrs.dtype)
        else:
            image_ftrs_vlm = None
        ...
        return {
            ...
            "image_feature_vlm": image_ftrs_vlm,
            "task": task,
            "duration": duration,
    ...
    def collate_func(self, batch):
        ...
        for sample in batch:
            ...
            if self.opt.vlm_based:
                tmp_image_feature_vlm = sample["image_feature_vlm"]
                image_feature_vlm_batch.append(tmp_image_feature_vlm)
        ...
        if self.opt.vlm_based:
            data["image_feature_vlm"] = torch.stack(image_feature_vlm_batch)
