            ...
            with tqdm(total=len(train_dataloader)) as pbar:
                for i_batch, batch in enumerate(train_dataloader):
                    ...
                    if args.explanation:
                        ...
                        alignment_loss = visual_alignment_loss
                        loss = loss_actions + args.lambda_duration * loss_duration + args.lambda_alignment * alignment_loss
            ...
            for dataset_name, gather_score in dataset_gather_scores_dict.items():
                if dataset_name in ["AiR-D", "OSIE"]:
                    ...
                    cur_metric = {
                        ...
                        "metrics/SS without Dur": gather_score["SS_score"][:, 0].mean(),
                        "metrics/SS with Dur": gather_score["SS_score"][:, 1].mean(),
                        "metrics/SSS without Dur": gather_score["SSS_score"][:, 0].mean() if gather_score["SSS_score"].shape[0] else 0.,
                        "metrics/SSS with Dur": gather_score["SSS_score"][:, 1].mean() if gather_score["SSS_score"].shape[0] else 0.,
                        "metrics/CC": gather_score["CC"].mean(),
                        "metrics/AUC": gather_score["AUC"].mean(),
                        "metrics/NSS": gather_score["NSS"].mean(),
                        "metrics/sAUC": gather_score["sAUC"].mean(),
